...
class Backup(GroupCog, description="Create Excel/SQL backups and restore from Excel or CSV. Mods only."):
    """
    Cog for backup slash commands: create Excel/SQL backups and restore
    from Excel or CSV. Restricted to moderators.
    """

    def __init__(self, bot: CustomClient):
        """Store bot reference, set up file rollers and interaction check."""
    ...
    async def _upsert_dataframe(self, session: Session, df: DataFrame, table_name: str) -> int:
        """
        Upsert DataFrame rows into the specified table. Updates existing
        rows by primary key; inserts new rows. Returns the number of rows processed.
        """
    ...
    def _disable_foreign_keys(self, session: Session):
        """
        Disable foreign key checks for the current session. Uses MySQL or
        SQLite syntax depending on the dialect.
        """
    ...
    async def _validate_foreign_keys(self, session: Session):
        """
        Validate foreign key constraints after a restore by flushing the
        session. Raises if any constraint is violated.
        """
...
async def teardown(_bot: CustomClient):
    _bot.remove_cog(Backup.__name__)  # remove_cog takes a string, not a class